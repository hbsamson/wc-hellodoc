...
function buildAppointmentNotifications(
  ...
  return consultations
    .flatMap((consultation) => {
      ...
      const subject = isDoctor ? 'patient appointment' : 'doctor appointment'
      ...
      if (consultation.status === 'scheduled' && scheduledAt >= now) {
        notifications.push({
          ...
          title: scheduledAt <= nextDay ? 'Upcoming appointment' : 'Scheduled appointment',
      ...
      if (consultation.status === 'completed') {
        notifications.push({
          ...
          time: formatRelativeTime(consultation.endedAt || consultation.updatedAt),
          href: `/consultations/${consultation.id}`,
          priority: 'normal',
        })
      }
